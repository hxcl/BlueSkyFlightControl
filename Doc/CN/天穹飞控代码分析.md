...
控制逻辑部分的代码位于 *PROJECT/SRC/CONTROL* 目录下，包含了遥控器、飞行状态控制、安全控制、电机控制、任务控制和路径控制等内容。下面将按照 *vFlightControlTask* 中的执行顺序分析各个控制内容的执行逻辑。
...
参数更新包括缓冲区中的更新和写入到 FLASH 两步，飞控提供了以下函数完成这一工作。当上位机的更新参数指令下发后，飞控会调用该函数刷新缓冲区的飞控参数，同时将 FLASH 保存计数器置位。
...
参数保存是强平台相关的程序。原版程序使用“FLASH 字节写入”的接口提供了抽象。

```c
bool Flash_WriteByte(uint32_t dest,uint8_t *src,uint32_t length)
```

然而这一抽象在相当多平台无法工作：许多 MCU 的 FLASH 不支持字节大小的编程。以 STM32H743 为例，其 FLASH 编程最小单位达到了 256 比特，与其闪存 ECC 特性有关。有必要提供一个更好的抽象接口。
